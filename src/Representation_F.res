module Make = (Token: Schema_intf.S, Dimension: Schema_intf.S, Scheme: Schema_intf.S) => {
  ...
  let rec to_JSON = t =>
    Js.Dict.fromList(list{
      ...
      ("tokens", t.tokens->List_rep2rep.to_JSON(Token.to_JSON)),
      ("dimensions", t.dimensions->List_rep2rep.to_JSON(Dimension.to_JSON)),
      ("schemes", t.schemes->List_rep2rep.to_JSON(Scheme.to_JSON)),
      ("subrepresentations", t.subrepresentations->List_rep2rep.to_JSON(to_JSON)),
    })->Js.Json.object_
  ...
  let rec of_JSON = json =>
    Js.Json.decodeObject(json)->Option.flatMap(dict => {
      ...
      let tokens = get_value("tokens", j => j->List_rep2rep.of_JSON(Token.of_JSON))
      let dimensions = get_value("dimensions", j => j->List_rep2rep.of_JSON(Dimension.of_JSON))
      let schemes = get_value("schemes", j => j->List_rep2rep.of_JSON(Scheme.of_JSON))
      let subrepresentations = get_value("subrepresentations", j =>
        j->List_rep2rep.of_JSON(of_JSON)
      )
